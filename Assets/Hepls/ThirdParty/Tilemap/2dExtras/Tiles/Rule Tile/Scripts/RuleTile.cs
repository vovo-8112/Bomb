...
namespace UnityEngine
{
	...
	public class RuleTile : TileBase
	{
        ...
        public Sprite m_WeightedSprite;
        public int m_Weight;
				...
				if (RuleMatches(rule, ref neighboringTiles, ref transform))
				{
					switch (rule.m_Output)
					{
						...
						case TilingRule.OutputSprite.Random:
                            ...
                            int maxLength = rule.m_Sprites.Length;
                            if (m_WeightedSprite != null)
                            {
                                maxLength += m_Weight;
                            }
                            int index = Random.Range(0, maxLength);

                            if (index < rule.m_Sprites.Length)
                            {
                                index = Mathf.Clamp(Mathf.FloorToInt(GetPerlinValue(position, rule.m_PerlinScale, 100000f) * rule.m_Sprites.Length), 0, rule.m_Sprites.Length - 1);
                                tileData.sprite = rule.m_Sprites[index];
                            }
                            else
                            {
                                tileData.sprite = m_WeightedSprite;
                            }
