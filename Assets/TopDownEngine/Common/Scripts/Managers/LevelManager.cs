...
namespace MoreMountains.TopDownEngine {
  ...
  public class LevelManager : MMSingleton<LevelManager>, MMEventListener<TopDownEngineEvent> {
    ...
    public Bounds LevelBounds {
      get {
        return (_collider == null) ? new Bounds() : _collider.bounds;
      }
    }
    ...
    protected virtual void InstantiatePlayableCharacters() {
      ...
      if (GameManager.Instance.StoredCharacter != null) {
        Character newPlayer = (Character) Instantiate(GameManager.Instance.StoredCharacter, _initialSpawnPointPosition,
          Quaternion.identity);
      ...
      if (PlayerPrefabs.Count() != 0) {
        foreach (Character playerPrefab in PlayerPrefabs) {
          ...
          var newPlayer =
              PhotonNetwork.Instantiate("MinimalGridCharacter", _initialSpawnPointPosition, Quaternion.identity)
            ;
    ...
    protected virtual IEnumerator SoloModeRestart() {
      ...
      if (CurrentCheckpoint != null) {
        ...
        Debug.LogWarning(
          "LevelManager : no checkpoint or initial spawn point has been defined, can't respawn the Player.");
      }
